...
class Chunk:
    ...
    def __init__(self,
        ...
        self.process_group = parallel_context.get_group()
        self.dp_world_size = self.parallel_context.get_world_size(ParallelMode.DATA)
        self.dp_local_rank = self.parallel_context.get_local_rank(ParallelMode.DATA)

        # the chunk size should be divisible by the dp degree
        if not keep_gathered:
            assert chunk_size % self.dp_world_size == 0
        self.shard_size = chunk_size // self.dp_world_size
        self.shard_begin = self.shard_size * self.dp_local_rank
        self.shard_end = self.shard_begin + self.shard_size
        self.valid_end = self.shard_size

        self.dtype = dtype
        device = init_device or ()
        ...
        self.shard_device = torch.device("cpu") if cpu_shard_init else torch.cuda.current_device()

        self.chunk_mem = self.chunk_size * self.chunk_temp.element_size()
        self.shard_mem = self.chunk_mem // self.dp_world_size
    ...
    def close_chunk(self):
        ...
        if self.chunk_temp.device.type == 'cpu':
            self.cuda_global_chunk = self.chunk_temp.to(torch.cuda.current_device())
    ...
    def shard_move(self, device: torch.device, force_copy: bool = False):
        ...
        if device.type == 'cuda':
            assert device == torch.cuda.current_device(), "can't move chunk to another device"

            if self.cuda_shard:
                return

            self.cuda_shard = self.cpu_shard.to(torch.cuda.current_device())
    ...
    def reduce(self):
        ...
        if self.dp_world_size == 1:
            ...
            dist.all_reduce(self.cuda_global_chunk, group=self.process_group)
        else:
            self.cuda_shard = torch.empty(self.shard_size, dtype=self.dtype, device=torch.cuda.current_device())

            input_list = list(torch.chunk(self.cuda_global_chunk, chunks=self.dp_world_size, dim=0))
            dist.reduce_scatter(self.cuda_shard, input_list, group=self.process_group)
    ...
    def __gather(self):
        if not self.is_gathered:
            ...
            gather_list = list(torch.chunk(input=self.cuda_global_chunk, chunks=self.dp_world_size, dim=0))
            dist.all_gather(gather_list, self.cuda_shard, self.process_group)
    ...
    def __paired_shard_move(self):
        ...
        assert self.cuda_shard is None
        temp = optim_chunk.cpu_shard.to(torch.cuda.current_device())
